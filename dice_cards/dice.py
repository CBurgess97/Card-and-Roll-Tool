...
def main() -> None:
    ...
    if not args:
        ...
        print("       roll table --print <file> [id]", file=sys.stderr)
        ...
        print("          --check    validate table file(s) against schema", file=sys.stderr)
        print("          --metadata show table file metadata", file=sys.stderr)
        print("          --print    pretty-print table contents", file=sys.stderr)
        sys.exit(1)
